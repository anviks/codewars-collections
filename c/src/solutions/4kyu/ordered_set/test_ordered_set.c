...
Test(Sample_Tests, Tests) {
  ...
  for (size_t i = 0; i < 10; i++) {
    ...
    cr_assert_not(set_includes(set, &a[i]),
                  "%d has not been added yet but was found in the set", a[i]);
    
    set_insert(set, &a[i]);
    
    n = set_size(set);
    cr_assert_eq(n, i + 1, "Size after insertion: %ld should be %ld", n, i + 1);
    cr_assert(set_includes(set, &a[i]),
              "%d has just been added but was not found in the set", a[i]);
  }
  ...
  for (size_t i = 0; i < 10; i++) {
    ...
    cr_assert(set_includes(set, &a[i]),
              "%d was added eariler but was not found in the set", a[i]);
    ...
    cr_assert_not(set_includes(set, &a[i]),
                  "%d has just been removed but was found in the set", a[i]);
  }
}
