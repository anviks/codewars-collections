...
TEST_CASE("the_multiply_function:should_pass_some_simple_assertions", "[the_multiply_function]") {
    do_test("2", "3", "6");
    do_test("30", "69", "2070");
    do_test("11", "85", "935");
}

TEST_CASE("the_multiply_function:should_pass_some_edge_assertions", "[the_multiply_function]") {
    do_test("2", "0", "0");
    do_test("0", "30", "0");
    do_test("0000001", "3", "3");
...
TEST_CASE("the_multiply_function:should_pass_some_advanced_fixed_assertions", "[the_multiply_function]") {
...
static void do_test(const char* a, const char* b, const char* axb) {
    ...
    char buff[200];
    snprintf(buff, 200, "a = \"%s\"\nb = \"%s\"\na * b = \"%s\"\nbut got \"%s\"", a, b, axb, actual);
    REQUIRE_MSG(strcmp(actual, axb) == 0, buff);
    free(actual);
}
